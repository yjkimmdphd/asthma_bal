...
# if nasal cell analysis, phen<-nphen. If bronchial analysis, phen<-bphen
#phen<-nphen # change this
phen<-bphen # change this
###########################################
...
col<-data.frame(matrix(nrow=sample.size,ncol=15))
colnames(col)<-c(source.cell.log,"batch","age","sex","Race","ethnicity")

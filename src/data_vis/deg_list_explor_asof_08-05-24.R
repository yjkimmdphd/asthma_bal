...
d8<-d8%>%mutate(
  analysis = rep("bronch.bal_aec_1_dich + Batch12346", nrow(d8)),
  anatomy = rep("bronch", nrow(d8)),
  `cont-dich` = rep("dich", nrow(d8))
)

# select the following:
# - bronch.bal_anc_m_0_cont+batch12346
# - bronch.bal_Eos_p_m_3_dich + Batch12346
# - nasal.blood_aec_m_0_cont+batch1234
# - bronch.bal_aec_1_dich + Batch12346
list_deg<-list(d3,d4,d6,d8)

# filter abs(log2FC) > 0.585 (at least 1.5 fold increase or decrease), then scale the fold change for clustering 
list_deg<-lapply(list_deg,function(df){df%>%filter(abs(log2FoldChange)>0.585)%>%mutate_at(vars(log2FoldChange),scale)})
print(lapply(list_deg,function(df){df%>%summarize(min=min(abs(log2FoldChange)),max=max(abs(log2FoldChange)))}))

deg_data<-rbind(list_deg[[1]],list_deg[[2]],list_deg[[3]],list_deg[[4]])

deg_file_path<-"./resources/processed_data/deg_list_as_of_08-05-24.csv"

# write.csv(deg_data,deg_file_path, row.names = FALSE)

deg_data<-read.csv(deg_file_path,row.names = NULL)
...
up_index<-match(nasal_kda_up_genes,rownames(log2fc_matrix))
down_index<-match(nasal_kda_down_genes,rownames(log2fc_matrix))

rownames(log2fc_matrix)<-rep("",nrow(log2fc_matrix))
rownames(log2fc_matrix)[up_index]<-nasal_kda_up_genes
rownames(log2fc_matrix)[down_index]<-nasal_kda_down_genes

# export the matrix
deg_matrix_asof080524<-write.table(log2fc_matrix,"./reports/local_only/deg_result_clustering/deg_matrix_asof080524.txt",sep="\t",row.names = TRUE,col.names = NA)

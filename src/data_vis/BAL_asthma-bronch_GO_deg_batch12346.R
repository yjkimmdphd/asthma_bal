...
###########################
## GO analysis  
# bronch expression ~ BAL cell counts + batch12346
###########################
library(tidyverse)
library(ggplot2)
library(stringr)
...
go_wgcna_folder<-file.path("./reports/local_only/wgcna/bronch/GO_wgcna_bronch")

go_deg_filelist<-list.files(path=go_deg_folder)
go_wgcna_filelist<-list.files(path=go_wgcna_folder)
...
go_deg_terms<-lapply(go_deg_terms,goSubset)
...
library(gridExtra)
for(i in 1:8){
  p<-ggplot(go_deg_terms[[i]], aes(x = Fold.Enrichment, y = X, fill = -log10(FDR))) +

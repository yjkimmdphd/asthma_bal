...
create_multipanel_mds <- function(normalized_counts, phen_var, col, title_prefix = "") {
...
analyze_mds_results <- function(normalized_counts, phen, title_prefix = "") {
  ...
  # Save results to CSV
  output_csv <- paste0("./reports/local_only/correlation_pca_trait/MDS_associations", 
...
# 9. Load count data
countdata_path <- "./resources/raw_data/MS_asthma/MS_asthma.batch12346.GRCh38.geneID_readcount.all_samples.QCed_final.txt"
counts <- if(file.exists(countdata_path)) {
  read.delim(countdata_path, check.names = FALSE)
}
genes <- counts[, "SampleID"]
# 10. Generate normalized counts
normalized_counts <- normalizeCounts(rbind(phen_bronch, phen_nasal), "comp2", "Batch", "~ comp2 + Batch")
normalized_counts_b <- normalizeCounts(phen_bronch, "comp2", "Batch", "~ comp2 + Batch")
normalized_counts_n <- normalizeCounts(phen_nasal, "comp2", "Batch", "~ comp2 + Batch")

# 11. Prepare variables for MDS color coding
...
# 14. Run MDS analysis for bronchial samples only
...
mds_results_bronch <- create_multipanel_mds(normalized_counts_b, phen_var_bronch, col_bronch, "Bronchial")
association_results_bronch <- analyze_mds_results(normalized_counts_b, phen_bronch, "Bronchial")

# 15. Run MDS analysis for nasal samples only
...
mds_results_nasal <- create_multipanel_mds(normalized_counts_n, phen_var_nasal, col_nasal, "Nasal")
association_results_nasal <- analyze_mds_results(normalized_counts_n, phen_nasal, "Nasal")

# 16. Print summary of key findings
...
top_assoc_bronch <- association_results_bronch[association_results_bronch$PC == "MDS1" & 
                                                 association_results_bronch$Significant == TRUE, ]
...
top_assoc_nasal <- association_results_nasal[association_results_nasal$PC == "MDS1" & 
                                               association_results_nasal$Significant == TRUE, ]

...
##############################################################
## Step 3: Hierarchical Clustering with 'hclust' for lcpm_down
##############################################################

## 3.1 clustering for lcpm_down
# Calculate the distance matrix
dist_mat_down <- dist(lcpm_down)

# Perform hierarchical clustering
hclust_result_down <- hclust(dist_mat_down, method = "complete")

hcut_level<-50
# explore the dendrogram with 'cutree' 
dendrogram_obj<-hclust_result_down%>%as.dendrogram
...
hcut_level<-50
gene_cluster_down<-cutree(hclust_result_down, h = hcut_level)

# Subset the gene expression matrix based on the cut clusters
cluster_order<-order(gene_cluster_down)
lcpm_down_hclust <- lcpm_down[cluster_order, ] # hclust ordered by cluster with custom cutoff h value 


# define color pallette for heatmap and row labels
deg_down<-deg_nb%>%filter(direction=="down")

# 3.2 compare lcpm_gene_names with deg_down gene names. reorder gene names in deg_down so that their order of gene names match that of lcpm_gene_names
# Find the order of genes in lcpm_down within deg_down
lcpm_gene_names<-rownames(lcpm_down_hclust)
order_genelist1 <- match(lcpm_gene_names, deg_down$X)

# Reorder genelist2 based on the order of genes in genelist1
deg_down_reordered <- deg_down[order_genelist1,]


# reorder col/samples based on blood Eos counts
new_sample_order<-match(phen_sub$SampleID,exist_samples)
new_lcpm_down_hclust<-lcpm_down_hclust[,new_sample_order]
head(new_lcpm_down_hclust)
head(phen_sub$SampleID)

# custom color palette for gene 
my_palette <- colorRampPalette(c("#7122F5","#FEFE62"))(n = 50) # custom heatmap pallette 

heatmap.2(new_lcpm_down_hclust,
          col = my_palette,
          trace = "none",  # Do not show row/column labels
          Colv=NA,
          scale = "row",  # Scale rows (genes)
          ...
          key.xlab = "Color Scale",          # X-axis label for the legend
          keysize = 0.7,  # Adjust this value for a more compact key
          main = "Gene Expression Heatmap of genes down-regulated in \nnasal_gene~blood_Eos and bronch_gene~BAL_neut",# heat map title
          RowSideColors = deg_down_reordered$type_col,
          ColSideColors = phen_sub$eos_col) 
...
###########################################---------------------###########################################
# sample type: nasal/bronchial. up-regulated genes only
# sample sorting: order of BAL ANC
###########################################---------------------###########################################
# 
bal_anc_phen<-phen[!is.na(phen$BAL_neut_ct),]
bal_anc_phen<-bal_anc_phen[,c("SampleID","ID","BAL_neut_ct_log")]
rownames(bal_anc_phen)<-c(1:nrow(bal_anc_phen))

bal_anc_phen$rna_sample<-c(rep("nasal",length(grep("^N", bal_anc_phen$SampleID))),
                           rep("bronch",length(grep("^B", bal_anc_phen$SampleID))))
phen_sub<-bal_anc_phen%>%group_by(rna_sample,BAL_neut_ct_log)%>%arrange(SampleID, rna_sample,BAL_neut_ct_log,.by_group=TRUE)%>%print(n=nrow(phen_sub))
...
phen_sub<-phen_sub%>%mutate(bal_anc_col=ifelse(BAL_neut_ct_log<0,"#E66100","#5D3A9B"))
...
################################################
## Step 2: Hierarchical Clustering with 'hclust'
################################################
...
heatmap.2(new_lcpm_up_hclust,
          ...
          main = "Gene Expression Heatmap of genes upregulated in \nnasal_gene~blood_Eos and bronch_gene~BAL_neut",# heat map title
          RowSideColors = deg_up_reordered$type_col,
          ColSideColors = phen_sub$bal_anc_col) 

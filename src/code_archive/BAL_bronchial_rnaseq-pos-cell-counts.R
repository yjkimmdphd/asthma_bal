...
c1<-filter_low_expressed_genes_method1(ct, 2, 4)
...
# count data = ct (readcount matrix)
# coldata = phen (phenotype data)
...
ct<-filter_readcounts(ct,c2) # low bcounts will be filtered using method 2: use TMM normalized lcpm as a cutoff point
...
if(dir.exists(deg.dir)){
  for(i in 1:10){
    ...
    write.csv(a,row.names=TRUE,file.path(deg.dir,paste0("deg_","bronch_poscell_","res",i,"_","design_",Sys.Date(),".csv"))) }
}
...
if(dir.exists(deg.dir)){
  ...
  write.csv(a,row.names=FALSE,file.path(deg.dir,paste("deg_bronch_poscell_analysis_input_",Sys.Date(),".csv",sep="")))
}
...
if(dir.exists(deg.dir)){
  ...
  write.csv(a,row.names=FALSE,file.path(deg.dir,paste("dds_bronch_poscell_res_summary_",Sys.Date(),".csv",sep="")))
}

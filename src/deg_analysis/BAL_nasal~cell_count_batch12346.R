...
if(dir.exists(deg.dir)){
  for(i in assay_index){
    a<-res.sig[[i]]
    b<-res[[i]]
    write.csv(a,row.names=TRUE,file.path(deg.dir,paste("deg","nasal","res_sig",i,deg.design[[i]],"cont_mt0",Sys.Date(),".csv",sep="_")))
    write.csv(b,row.names=TRUE,file.path(deg.dir,paste("deg","nasal","res_all",i,deg.design[[i]],"cont_mt0",Sys.Date(),".csv",sep="_")))
  }
}

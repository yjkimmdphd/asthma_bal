...
bphen<-bphen%>%mutate(bld_AEC_more_150 = blood_eos>150,
                      bld_AEC_more_300 = blood_eos>300,
                      bld_AEC_more_500 = blood_eos>500)
bphen<-bphen%>%filter(BAL_eos_p>0)
# bphen<-bphen%>%filter(BAL_eos_p==0)
phen<-bphen
...
var_dichot_blood<-c("bld_AEC_more_150",
                    "bld_AEC_more_300",
                    "bld_AEC_more_500")
...
var_to_test<-c(var_dichot_blood) # select continuous and categorical variables 
var_to_test_bld<-var_to_test[c(grep("blood",var_to_test),grep("bld",var_to_test))] # blood cell counts among the variables to test 
var_to_test_res<-c(paste(c(var_dichot_blood),"TRUE",sep="")) # used for result() 
...
deg.folder<-paste("deg","dich_ge0",Sys.Date(),sep="_")

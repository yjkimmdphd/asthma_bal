################
# gsea data prep
# select for genes in the WGCNA ivory module
################
library(tidyverse)
library(vsn)
library(DESeq2)
...
# find list of genes in the wgcna ivory module
wgcna_genes<-unlist(read.table("./reports/local_only/wgcna/bronch/module-gene_list/ivorybatch12346.txt"))
names(wgcna_genes)<-NULL
head(wgcna_genes)
...
# Subset vsd by rows (genes) matching wgcna_genes
vsd_subset <- vsd[rownames(vsd) %in% wgcna_genes, ]
assay(vsd_subset)
...
write.table(
  assay(vsd_subset),
  "./resources/processed_data/gsea/scaled_count_vsd_wgcna-ivory2024-12-27-2.txt",
  sep = "\t",
  row.names = TRUE, 
  col.names = NA
)

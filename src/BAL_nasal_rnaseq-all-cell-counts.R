...
#######################################################################
nID<-paste0("N",phenotype$ID) # N*** indicates bronchial sample ID, sequence data is not available for all as of 2023-10-04
...
bal.all.e.p<-subset_phenotype(nphen,source.cell.all,"BAL_eos_p","SampleID","BAL_eos_p_log", "Batch")
##df with bal neut>=0
bal.all.n.c<-subset_phenotype(nphen,source.cell.all,"BAL_neut_ct","SampleID","BAL_neut_ct_log","Batch")
bal.all.n.p<-subset_phenotype(nphen,source.cell.all,"BAL_neut_p","SampleID","BAL_neut_p_log","Batch")
##df with bal WBC>=0
bal.all.w.c<-subset_phenotype(nphen,source.cell.all,"BAL_wbc","SampleID","BAL_wbc_log","Batch")

##df with serum_Eos>=0
ser.all.e.c<-subset_phenotype(nphen,source.cell.all,"blood_eos","SampleID","blood_eos_log", "Batch")
ser.all.e.p<-subset_phenotype(nphen,source.cell.all,"blood_eos_p","SampleID","blood_eos_p_log","Batch")
##df with serum_Neut>=0
ser.all.n.c<-subset_phenotype(nphen,source.cell.all,"blood_neut","SampleID","blood_neut_log","Batch")
ser.all.n.p<-subset_phenotype(nphen,source.cell.all,"blood_neut_p","SampleID","blood_neut_p_log","Batch")
##df with serum WBC>=0
ser.all.w.c<-subset_phenotype(nphen,source.cell.all,"blood_wbc","SampleID","blood_wbc_log","Batch")
...
if(dir.exists(deg.dir)){
  for(i in 1:10){
    ...
    write.csv(a,row.names=TRUE,file.path(deg.dir,paste0("deg_","nasal_allcell_","res",i,"_","design_",Sys.Date(),".csv"))) }
}
...
if(dir.exists(deg.dir)){
  ...
  write.csv(a,row.names=FALSE,file.path(deg.dir,paste("deg_nasal_allcell_analysis_input_",Sys.Date(),".csv",sep="")))
}
...
if(dir.exists(deg.dir)){
  ...
  write.csv(a,row.names=FALSE,file.path(deg.dir,paste("dds_nasal_allcell_res_summary_",Sys.Date(),".csv",sep="")))
}

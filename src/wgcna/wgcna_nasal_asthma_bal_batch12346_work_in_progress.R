...
png("./reports/local_only/wgcna/nasal/sampleTree_nasal.png", width = 800, height = 600)
par(cex = 0.6)
par(mar = c(0, 4, 2, 0))
plot(
  sampleTree,
...
plot(METree)
abline(h=0.1, col = "red")  #a height of h corresponds to correlation of 1.00 - h (i.e.,  all of the modules which are more than 85% similar if h=0.15.)

merge <- mergeCloseModules(expression.data, ModuleColors, cutHeight = 0.1) # merge the modules which are below the threshold
...
output_folder<-file.path("./reports/local_only/wgcna/nasal")
write.table(df_genelist, file = file.path(output_folder,"nasal_wgcna_genelist_batch12346.txt"), sep = "\t", row.names = FALSE, col.names = TRUE, na = "", quote = FALSE)

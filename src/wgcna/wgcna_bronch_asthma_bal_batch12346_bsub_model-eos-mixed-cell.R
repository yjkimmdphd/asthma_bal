...
# set folders
normalized_count_table_path_1 <- "./resources/processed_data/normalized_gene_count/normalization_comp1_mixed_cell"
normalized_count_table_path_2 <- "./resources/processed_data/normalized_gene_count/normalization_comp2_eos_high_low"

# Load cell count table
# normalization of the counts used the parameters specified below
normalized_count_table_path<-normalized_count_table_path_1
...
if (file.exists(file.path(normalized_count_table_path,"normalized_gene_count_bronch_vsd_batch-corrected.txt"))) {
  gene_counts <- read.table(normalized_count_table_path, 
                       ...
                       row.names = 1, 
                       sep = "\t")
  normalization_parameter_lines<-readLines(file.path(normalized_count_table_path,"normalization_parameters.txt"))
  kv_lines <- normalization_parameter_lines[grepl("^[^#=]+:\\s", normalization_parameter_lines)]
}
...
output_folder <- file.path("./reports/local_only/wgcna/bronch/output/comp1",Sys.Date())
...
# Create a named list of parameters for count normalization
params <- setNames(
  trimws(sub("^[^:]+:\\s*", "", kv_lines)),
  trimws(sub(":.*", "", kv_lines))
)


# Write parameters to a file in key: value format
param_output_path <- file.path(output_folder, "normalization_parameters_cleaned.txt")

###
# 1. Create a new format for expression data
###
expression <- as.data.frame(t(gene_counts))
...
# Pick soft threshold
spt <- pickSoftThreshold(expression.data, powerVector = c(1:20), verbose = 5)

# Automatically select the lowest power where R^2 > 0.8
sft_df <- spt$fitIndices
sft_df <- sft_df[order(sft_df$Power), ]
selected_power <- sft_df$Power[which(sft_df$SFT.R.sq > 0.8)[1]]

# Fallback if no power meets threshold
if (is.na(selected_power)) {
  warning("No power met R^2 > 0.8; using power with max R^2 instead.")
  selected_power <- sft_df$Power[which.max(sft_df$SFT.R.sq)]
}

softPower <- selected_power

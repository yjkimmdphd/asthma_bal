...
merge_modcol_new<-as.character(unlist(read.table(file.path(output_folder,"mergedColors_h-018.txt"), row.names=1, header=TRUE)))
merge_ME_new<-read.table(file.path(output_folder,"mergedMEs_h-018.txt"), row.names=1, header=TRUE)

mergedColors<-merge_modcol_new
mergedMEs<-merge_ME_new
...
write.table(
  ...
  file = file.path(output_folder, "top_kWithin_by_module_deg_overlap_h-018.txt"),
  sep = "\t",

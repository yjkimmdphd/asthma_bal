library(KDA)
library(dplyr)
kdapath<-file.path("C:/Users/kimyo/Dropbox/Research/R_packages/KDA_0.2.2.tar/KDA/R")
sapply(file.path(kdapath,list.files(kdapath)), source)
...
##----------------------------------------------------------------------------------------------------
# Input: 
#   1. gene list: nasal_blood_eos ="deg_Nasal_poscells_~ blood_eos_log + Batch_res_6_2024-01-09_.csv"
#   2. causal network: nasal_bn=""; 
...
if ( directed )
{
  outputDir <- "./reports/asthma_nasal_KeyDrivers/"
} else
{
  outputDir <- "./reports/asthma_nasal_KeyDrivers-undirected/"
}
...
fname <- "kda_nasal_asthma_blood_eos"
fname <- paste( fname , "_L" , layer , sep = "" )

################################################################################################
# 2. read in gene lists

listMatrix <- nasal_blood_eos
dim( listMatrix )
